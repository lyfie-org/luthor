...
import {
  ...
  GeneratedRegistry,
} from "../app/(docs)/lib/types";

/**
 * Code registry generator
 * Scans docs/**\/codes.tsx, docs/**\/examples/**, and components/templates/** to build a highlighted registry
 */
class CodeRegistryGenerator {
  ...
  /**
   * Find codes.tsx files in docs/** and components/templates/**
   */
  ...
  /**
   * Find example files in docs/** and components/templates/** (recursive)
   */
  private findExampleFiles(): Array<{
    relativePath: string;
    fullPath: string;
    language: string;
  }> {
  ...
  /**
   * Load snippets from a codes.tsx module
   */
  private async loadSnippetsFromFile(
    filePath: string,
  ): Promise<RegisteredCodeSnippet[]> {
    try {
  ...
  /**
   * Render a snippet with Shiki
   */
  private async processSnippet(
    snippet: RegisteredCodeSnippet,
  ): Promise<string> {
    return await codeToHtml(snippet.code, {
      lang: snippet.language,
      theme: "github-dark",
      transformers:
        snippet.highlightLines && snippet.highlightLines.length > 0
          ? [
              {
                line(node, line) {
  ...
  /**
   * Render an example file with Shiki
   */
  private async processExampleFile(
    fullPath: string,
    language: string,
  ): Promise<string> {
  ...
  /**
   * Generate the static registry output
   */
  async generateRegistry(): Promise<void> {
    ...
    console.log(
      `📁 Found ${codesFiles.length} codes.tsx files:`,
      codesFiles.map((f) => f.relativePath),
    );
    console.log(
      `📁 Found ${exampleFiles.length} example files:`,
      exampleFiles.map((f) => f.relativePath),
    );
    ...
    for (const file of codesFiles) {
      ...
      console.log(
        `✅ Loaded ${snippets.length} snippets from ${file.relativePath}`,
      );
    }
    ...
    for (const file of exampleFiles) {
      ...
      const highlighted = await this.processExampleFile(
        file.fullPath,
        file.language,
      );
...
`;
  ...
  /**
   * Generate a runtime loader that imports all codes.tsx modules
   */
  private generateRuntimeLoader(
    codesFiles: Array<{ relativePath: string; fullPath: string }>,
  ): void {
    const loaderPath = path.join(this.outputDir, "codes-loader.ts");

    // Convert file paths to import paths relative to the loader
    const importStatements = codesFiles
      .map((file, index) => {
    ...
    const moduleExports = codesFiles
      .map((file, index) => `  codes${index}`)
      .join(",\n");
