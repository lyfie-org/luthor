  ...
  },
  {
    ...
    "sections": [
      ...
      },
      {
        ...
        "level": 2,
        "text": "tsx uploadToCdn(file)} gifUploadHandler={async (file) = uploadGifToMediaStore(file)} / If gifUploadHandler is not provided, GIF file uploads use imageUploadHandler . For production handlers, return a persistent URL from your storage service. Returning blob: URLs from handlers is fine for quick prototypes, but dev StrictMode remount cycles can revoke blob URLs and cause temporary ERR FILE NOT FOUND preview errors."
      },
      {
  ...
  },
  {
    ...
    "sections": [
      ...
      },
      {
        ...
        "level": 3,
        "text": "Two triggers open a caret-anchored dropdown, and both are fed by the adapter: Trigger Menu Fed by Inserts ------- ---------- ----------------------- -------------------------- [[ note links searchNotes(q) a [[Note]] wikilink @ mentions searchUsers(q) plain @username text Both are host-gated: with no adapter (or, for @ , no searchUsers ) the trigger is silent and no menu renders, so the editor never offers a suggestion it cannot honour. Keyboard-driven throughout — Escape closes, arrows move, Enter/Tab select, and a click outside dismisses. The @ trigger matches the mention rule byte-for-byte: an @ only opens the menu at the start of a block or after whitespace, ( , or [ , and the query is [A-Za-z0-9][A-Za-z0-9. -]{0,63} . bea@example.com never opens it. Mentions are written as plain text , not a node — nothing to serialize, nothing that can rewrite the body on save — and hosts detect them by scanning the markdown (see getMentions() ). Both triggers open on the bare trigger by default ( minQueryLength: 0 ), so typing @ or [ shows the host's first page immediately. Raise the floor per trigger if an unfiltered list is noise — see [Tuning the typeahead . Where they open. Both triggers only open inside a block that can carry a ^id block anchor — paragraph, heading, quote, and list item. That is not a style choice: a host resolves a mention to the anchor of the block it sits in, so a mention typed anywhere else could never be delivered. The trigger container set and the block-anchor stamping set are derived from one exported source of truth ( ANCHORABLE BLOCK TYPES in @lyfie/luthor-headless ), so they cannot drift apart. Code blocks and tables are excluded in both."
      },
      {
        "heading": "Tuning the typeahead",
        "id": "tuning-the-typeahead",
        "level": 3,
        "text": "The typeahead prop is the host's seam over both triggers. Every field is optional and defaults to the shipped behaviour, so omitting it changes nothing. tsx Field Default What it does ----------------------- ---------------------- -------------------------------------------------------------- minQueryLength 0 Characters required after the trigger before the menu opens. offset { x: 0, y: 8 } Menu position relative to the caret. title Mention / Link note Menu heading (localisation, product vocabulary). emptyLabel see above Shown when the host's search returns nothing. disabled false Unregisters the trigger — the character stays ordinary text. searchDebounceMs 120 Debounce before a query reaches the adapter. Shared. Only the length floor is configurable. The syntax rules are fixed at every setting: a username's legal characters still close the @ menu on the first character it cannot accept, and [[ still never opens after ! or once the link is closed or aliased. disabled is the switch for a surface that has no people directory, or a deployment that does not use note links — previously the only way to suppress a trigger was not to mount the editor. It drops the trigger extension and the matching suggestion provider; the embed nodes (wikilink rendering, media, anchors) stay registered, so existing [[Note]] links in the body keep working. Menu size is CSS, not a prop: override --luthor-typeahead-menu-max-height and --luthor-typeahead-menu-width on your own scope."
      },
      {
        "heading": "Usage",
        "id": "usage",
    ...
    "updatedAt": "2026-08-13T10:09:55.837Z",
    "package": "luthor",
    "docType": "reference",
    "surface": "preset",
    "keywords": [
