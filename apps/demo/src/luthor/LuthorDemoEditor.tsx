...
import { TextBolderIcon, TextItalicIcon, TextUnderlineIcon, TextStrikethroughIcon, List, ListIcon, SkipForwardIcon, RewindIcon, Sun, Moon, Image as ImageIcon, AlignLeft, AlignCenterHorizontalIcon, AlignRight, Upload, Link, LinkBreakIcon, Minus, Code, Terminal, Table as TableIcon, FileCode, Eye, Pencil, Command, TrayArrowUpIcon, QuotesIcon, TextIndentIcon, TextOutdentIcon, LinkIcon, ListBulletsIcon, AlignLeftIcon, AlignRightIcon } from "@phosphor-icons/react";
import { Select, Dropdown, Dialog } from "./components/components";
import {
...
import { FastForwardCircleIcon } from "@phosphor-icons/react/dist/ssr";
      ...
      {isImageSelected ? (
        ...
        <>
          ...
          <button onClick={() => activeStates.isLink ? commands.removeLink() : commands.insertLink()} className={`luthor-toolbar-button ${activeStates.isLink ? "active" : ""}`} title={activeStates.isLink ? "Remove Link" : "Insert Link"}>
            {activeStates.isLink ? <LinkBreakIcon size={14} /> : <LinkIcon size={14} />}
          </button>
          <div className="w-px h-6 bg-border mx-1" />
      ...
      <div className="luthor-toolbar">
        ...
        {hasExtension("list") && (
          <div className="luthor-toolbar-section">
            ...
            <button onClick={() => commands.toggleOrderedList()} className={`luthor-toolbar-button ${activeStates.orderedList ? "active" : ""}`} title="Numbered List"><ListBulletsIcon size={16} /></button>
        ...
        {hasExtension("image") && (
          <div className="luthor-toolbar-section">
            ...
            {activeStates.imageSelected && (
              ...
              >
                <button className="luthor-dropdown-item" onClick={() => { handlers.setAlignment("left"); setShowAlignDropdown(false); }}><AlignLeftIcon size={16} /> Align Left</button>
                ...
                <button className="luthor-dropdown-item" onClick={() => { handlers.setAlignment("right"); setShowAlignDropdown(false); }}><AlignRightIcon size={16} /> Align Right</button>
        ...
        {hasExtension("history") && (
          <div className="luthor-toolbar-section">
            ...
            <button onClick={() => commands.redo()} disabled={!activeStates.canRedo} className="luthor-toolbar-button" title="Redo (Ctrl+Y)"><FastForwardCircleIcon size={16} /></button>
          </div>
        )}
